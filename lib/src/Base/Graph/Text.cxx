  ...
  : DrawableImplementation(data, legend)
{
  ...
  if (textPosition > 4)
  {
    throw InvalidDimensionException(HERE) << "Expected position is 1 (below), 2 (left), 3 (above) or 4 (right), got " << textPosition;
  }
...
void Text::setTextPositions(const Indices & textPositions)
{
  ...
  {
    if (textPositions[i] > 4)
    {
      throw InvalidDimensionException(HERE) << "Expected position is 1 (below), 2 (left), 3 (above) or 4 (right), got " << textPositions[i] << " at index " << i;
    }
  }
